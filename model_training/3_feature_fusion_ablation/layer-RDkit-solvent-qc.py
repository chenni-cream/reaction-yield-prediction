...
def evaluate_fusion(
    ...
    X = np.concatenate([layered_feats, rdkit_feats, solv_feats, qc_feats], axis=1)
    y = rxn_df[TARGET_COLUMN].to_numpy(dtype=np.float32)

    print(
        f"    特征维度: Layered={layered_feats.shape[1]}, RDKit={rdkit_feats.shape[1]}, "
        f"Solvent={solv_feats.shape[1]}, QC={qc_feats.shape[1]}, 总={X.shape[1]}"
    )
...
def analyze_importance(
    ...
    types = ["Layered"] * n_layered + ["RDKit"] * n_rdkit + ["Solvent"] * n_solv + ["QC"] * n_qc
    components = (
    ...
    parts = []
    for t in ["Layered", "RDKit", "Solvent", "QC"]:
...
def main() -> None:
    ...
    output_root = script_dir.parent / "ckpt-layered-rdkit-solvent-qc-fusion"
    ...
    print(f"\n{'=' * 70}")
    print("Layered FP + RDKit + Solvent + QC Descriptors 融合训练")
    print(f"{'=' * 70}")
    ...
    pd.DataFrame(fusion_folds).to_csv(
        results_dir / "layered_rdkit_solvent_qc_fusion_fold_metrics.csv", index=False
    )
    pd.DataFrame(fusion_summary).to_csv(
        results_dir / "layered_rdkit_solvent_qc_fusion_summary.csv", index=False
    )
    ...
    # ── 对比汇总表 ──
    print(f"\n{'=' * 80}")
    print("对比汇总: Layered+RDKit 基线 vs Layered+RDKit+Solvent+QC 融合")
    print(f"{'=' * 80}")
