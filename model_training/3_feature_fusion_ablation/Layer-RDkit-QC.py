...
def load_qc_gz(gz_path: Path) -> pd.DataFrame:
    """加载单个 gz 压缩的 QC 描述符文件，返回 DataFrame（smile 列 + 特征列）"""
...
def build_qc_lookup(
    ...
    """
    为每个分子列构建 SMILES -> QC 特征向量的查找表。
    round1 目录缺少 Product，由 round2 目录的 Product 补充。
    """
    file_map = {
    ...
    for col, filename in file_map.items():
        ...
        # round2 目录（qm_desc-morfeus）：全部加载
...
def build_qc_features(
    ...
    """为整个 DataFrame 构建 QC 特征矩阵，每个分子列 10 个特征，共 50 个"""
    parts = []
...
def analyze_importance(
    ...
    types = ["Layered"] * n_layered + ["RDKit"] * n_rdkit + ["QC"] * n_qc
    components = (
...
def main() -> None:
    ...
    output_root = script_dir.parent / "ckpt-layered-rdkit-qc-fusion"
    ...
    print(f"\n{'=' * 70}")
    print("Layered FP + RDKit + QC Descriptors 融合训练")
    print(f"{'=' * 70}")
    ...
    for rxntype in sorted(rxn_groups.keys()):
        ...
        print(f"    QC 特征矩阵: {qc_feats.shape}")
    ...
    pd.DataFrame(fusion_folds).to_csv(
        results_dir / "layered_rdkit_qc_fusion_fold_metrics.csv", index=False
    )
    pd.DataFrame(fusion_summary).to_csv(
        results_dir / "layered_rdkit_qc_fusion_summary.csv", index=False
    )

    # 加载已有的 Layered+RDKit 基线结果进行对比
    baseline_df = None
    ...
    # ── 对比汇总表 ──
    print(f"\n{'=' * 80}")
    print("对比汇总: Layered+RDKit 基线 vs Layered+RDKit+QC 融合")
    print(f"{'=' * 80}")
