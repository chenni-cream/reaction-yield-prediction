# LightGBM: Fusion Training of Avalon Fingerprint and Extra-RDKit Descriptors
# Classified by rxntype, using 5-fold cross-validation
# Compared with the pure Avalon FP baseline, evaluating the gains brought by RDKit features
import gzip
...
def avalon_fp(mol: Chem.Mol, n_bits: int) -> np.ndarray:
    if not HAS_AVALON:
        raise ImportError("rdkit.Avalon 不可用")
...
def build_avalon_features(df: pd.DataFrame) -> np.ndarray:
...
def evaluate_fusion(
    ...
    X = np.concatenate([avalon_feats, rdkit_feats], axis=1)
    y = rxn_df[TARGET_COLUMN].to_numpy(dtype=np.float32)

    print(f"    特征维度: Avalon={avalon_feats.shape[1]}, RDKit={rdkit_feats.shape[1]}, "
          f"总={X.shape[1]}")
...
def analyze_importance(
    ...
    n_avalon = avalon_feats.shape[1]
    n_rdkit = rdkit_feats.shape[1]
    types = ["Avalon"] * n_avalon + ["RDKit"] * n_rdkit
    components = (
    ...
    print(f"    特征重要性占比: Avalon={type_imp.get('Avalon', 0)/total_imp*100:.1f}%, "
...
def main() -> None:
    ...
    output_root = script_dir.parent / "ckpt-avalon-rdkit-fusion"
    ...
    for rt in sorted(rxn_groups):
        ...
        print(f"  rxntype={rt}: 期望 {expected}, 实际 {actual}, 特征数 {rdkit_arr.shape[1]} [{status}]")
    ...
    print(f"\n{'='*70}")
    print("Avalon FP + RDKit Descriptors 融合训练")
    print(f"{'='*70}")
    ...
    for rxntype in sorted(rxn_groups.keys()):
        ...
        print(f"  rxntype={rxntype} | R2: {summary_row['r2_mean_pm_sd']}"
    ...
    pd.DataFrame(fusion_folds).to_csv(results_dir / "avalon_rdkit_fusion_fold_metrics.csv", index=False)
    pd.DataFrame(fusion_summary).to_csv(results_dir / "avalon_rdkit_fusion_summary.csv", index=False)
    ...
    # ── 对比汇总表 ──
    print(f"\n{'='*80}")
    print("对比汇总: 纯 Avalon 基线 vs Avalon+RDKit 融合")
    print(f"{'='*80}")
