...
def analyze_importance(
	...
	types = ["Layered"] * n_layered + ["RDKit"] * n_rdkit
	components = (
	...
	print(
		f"    特征重要性占比: Layered={type_imp.get('Layered', 0) / total_imp * 100:.1f}%, "
...
def main() -> None:
	...
	output_root = script_dir.parent / "ckpt-layered-rdkit-fusion"
	...
	print(f"\n{'=' * 70}")
	print("Layered FP + RDKit Descriptors 融合训练")
	print(f"{'=' * 70}")
	...
	pd.DataFrame(fusion_folds).to_csv(results_dir / "layered_rdkit_fusion_fold_metrics.csv", index=False)
	pd.DataFrame(fusion_summary).to_csv(results_dir / "layered_rdkit_fusion_summary.csv", index=False)
	...
	# ── 对比汇总表 ──
	print(f"\n{'=' * 80}")
	print("对比汇总: 纯 Layered 基线 vs Layered+RDKit 融合")
	print(f"{'=' * 80}")
