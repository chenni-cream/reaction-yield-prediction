...
def main():
    ...
    # ── Step 1: 全量 LayeredFingerprint+RDKit ──
    print(f"\n{'='*70}")
    print("Step 1: LayeredFingerprint + RDKit 全量 (11290 维)")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print("Step 2: LayeredFingerprint + Top-K RDKit 消融")
    print(f"{'='*70}")
    ...
    for rxntype in sorted(rxn_groups.keys()):
        ...
        for k in k_values:
            ...
            all_indices = np.concatenate([
                np.arange(n_layered),       # Layered 全部保留
    ...
    results_df.to_csv(results_dir / "ablation_layered_topk_results.csv", index=False)
    ...
    # 保存全局最优配置
    config_avg.to_csv(results_dir / "ablation_layered_global_k_ranking.csv")

    best_df = best_sub[["rxntype", "n_samples", "r2_mean", "r2_sd", "rmse_mean", "mae_mean", "n_features"]].copy()
    best_df.to_csv(results_dir / "ablation_layered_best_global_k_results.csv", index=False)

    print(f"\n结果保存: {results_dir / 'ablation_layered_topk_results.csv'}")
    print(f"排名表:   {results_dir / 'ablation_layered_global_k_ranking.csv'}")
    print(f"最优结果: {results_dir / 'ablation_layered_best_global_k_results.csv'}")
